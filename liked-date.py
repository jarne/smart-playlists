...
def sync_playlist(user_id, playlist_name, track_ids):
    print(f"Processing: {playlist_name} with {len(track_ids)} tracks.")
...
def main():
    ...
    # Group tracks by year-month and by year of 'added_at'
    ...
    for item in liked_tracks:
        ...
        tracks_by_year[year_key].append(item["track"]["id"])
    print(
        f"Grouped tracks into {len(tracks_by_month)} months and {len(tracks_by_year)} years."
    )
    ...
    for month, track_ids in tracks_by_month.items():
        ...
        sync_playlist(user_id, playlist_name, track_ids)

    # For each year, create or get playlist and add tracks
    for year, track_ids in tracks_by_year.items():
